 ...
 */
namespace common\components\workerman;
require_once __DIR__ . '/Lib/Constants.php';

use common\components\workerman\Events\EventInterface;
use common\components\workerman\Connection\ConnectionInterface;
use common\components\workerman\Connection\TcpConnection;
use common\components\workerman\Connection\UdpConnection;
use common\components\workerman\Lib\Timer;
use common\components\workerman\Events\Select;
use Exception;
...
class Worker
{
    ...
    protected static $_availableEventLoops = array(
        'libevent' => '\common\components\workerman\Events\Libevent',
        'event'    => '\common\components\workerman\Events\Event',
        'swoole'   => '\common\components\workerman\Events\Swoole'
    );
    ...
    protected static function installSignal()
    {
        ...
        // stop
        pcntl_signal(SIGINT, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // graceful stop
        pcntl_signal(SIGTERM, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // reload
        pcntl_signal(SIGUSR1, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // graceful reload
        pcntl_signal(SIGQUIT, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // status
        pcntl_signal(SIGUSR2, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // connection status
        pcntl_signal(SIGIO, array('\common\components\workerman\Worker', 'signalHandler'), false);
        // ignore
    ...
    protected static function reinstallSignal()
    {
        ...
        static::$globalEvent->add(SIGINT, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
        // reinstall graceful stop signal handler
        static::$globalEvent->add(SIGTERM, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
        // reinstall reload signal handler
        static::$globalEvent->add(SIGUSR1, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
        // reinstall graceful reload signal handler
        static::$globalEvent->add(SIGQUIT, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
        // reinstall status signal handler
        static::$globalEvent->add(SIGUSR2, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
        // reinstall connection status signal handler
        static::$globalEvent->add(SIGIO, EventInterface::EV_SIGNAL, array('\common\components\workerman\Worker', 'signalHandler'));
    }
    ...
    protected static function getEventLoopName()
    {
        ...
        if ($loop_name) {
            if (interface_exists('\React\EventLoop\LoopInterface')) {
                switch ($loop_name) {
                    case 'libevent':
                        static::$eventLoopClass = '\common\components\workerman\Events\React\ExtLibEventLoop';
                        break;
                    case 'event':
                        static::$eventLoopClass = '\common\components\workerman\Events\React\ExtEventLoop';
                        break;
                    default :
                        static::$eventLoopClass = '\common\components\workerman\Events\React\StreamSelectLoop';
                        break;
                }
            } else {
            ...
            static::$eventLoopClass = interface_exists('\React\EventLoop\LoopInterface')? '\common\components\workerman\Events\React\StreamSelectLoop':'\common\components\workerman\Events\Select';
        }
        return static::$eventLoopClass;
    }
    ...
    {
        Timer::add(1, "\\common\\components\\Workerman\\Worker::checkWorkerStatusForWindows");
    ...
    public static function stopAll()
    {
        ...
        if (static::$_masterPid === posix_getpid()) {
            ...
            Timer::add(1, "\\common\\components\\Workerman\\Worker::checkIfChildRunning");
        ...
        if (!$this->_mainSocket) {
            ...
            if (!isset(static::$_builtinTransports[$scheme])) {
                ...
                if (!class_exists($this->protocol)) {
                    $this->protocol = "\\common\\components\\Workerman\\Protocols\\$scheme";
                    if (!class_exists($this->protocol)) {
    ...
    public function run()
    {
        ...
        register_shutdown_function(array("\\common\\components\\Workerman\\Worker", 'checkErrors'));
